...
fn dump_mpi(
    local: &Vec<f64>, rank: i32, procs: i32, root_process: &Process<SystemCommunicator>,
    global: &mut Vec<f64>, index: &mut usize
...
fn onestep(lattice: &mut Vec<f64>, orig: &mut Vec<f64>, h: f64, rank: i32, procs: i32, world: &SystemCommunicator) {
    ...
    *orig = lattice.clone();
    ...
    // 右端を右に送って、左端を左から受け取る
    p2p::send_receive_into(&lattice[size - 2], &right_process, &mut orig[0], &left_process);
    // 左端を左に送って、右端を右から受け取る
    p2p::send_receive_into(&lattice[1], &left_process, &mut orig[size - 1], &right_process);
...
#[allow(dead_code)]
fn uniform_heating(lattice: &mut Vec<f64>, rank: i32, procs: i32, world: &SystemCommunicator) -> Result<(), Box<dyn std::error::Error>> {
    let h = 0.2;
    let q = 1.0;
    let mut index = 0;
...
#[allow(dead_code)]
fn fixed_temperature(lattice: &mut Vec<f64>, rank: i32, procs: i32, world: &SystemCommunicator) -> Result<(), Box<dyn std::error::Error>> {
    let h = 0.01;
    let q = 1.0;
